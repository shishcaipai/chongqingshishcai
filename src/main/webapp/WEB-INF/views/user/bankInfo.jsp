					...
					<div class="tc_yhzx_k">
						...
						<div class="p_nr">
							...
							<table cellpadding="6" cellspacing="6" class="table_u_all">
								...
								<tr>
									<td class="tikuan_a2" align="right">支付宝账号(可选)：</td>
								...
								<tr>
									...
									<td class="tikuan_a3"><input id="bankName" type="text"
										class="zlxg_srk" size="30" value="${memberUser.bankName}" /></td>
								</tr>
								<tr>
								...
								<c:if test="${empty memberUser.zfbCode}">
									<tr>
										<td class="tikuan_a2" align="right">提现密码：</td>
										<td class="tikuan_a3"><input id="password"
											type="password" class="zlxg_srk" size="30" /></td>
									</tr>
									<tr>
										<td class="tikuan_a2"></td>
										<td class="tikuan_a3"><input id="btnCommit" type="submit"
											value="提&nbsp;交" name="Submit" onclick="bankCodeSubmit()" />
									</tr>
								</c:if>
							</table>
							<%-- </c:otherwise>
	...
	<script type="text/javascript">
	    function bankCodeSubmit(){
		  /*   if($("#bankCode").val().trim()==''){
			...
			/* if(!$("#zfbCode").val().trim()==''||!validateEmail($("#zfbCode").val().trim())
			...
			$.ajax({
				...
				success : function(result) {
					alert(result);
					if('银行账号保存成功'==result){
					window.location.href='<%=basePath%>money/apply'; 
				}
				}
			});
		}
