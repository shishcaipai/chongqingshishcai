...
fw.isch = /chrome/i.test(navigator.userAgent);
fw.brower = {
	name : fw.isie ? "IE" : fw.isff ? "Firefox" : fw.isop ? "Opear" :fw.isch ? "chrome": "other",
	version : navigator.userAgent.match(/(?:msie|firefox|opera|chrome)[ |\/]([\d\.]+)/i)[1]
};
fw.isie6 = fw.isie && fw.brower.version=="6.0";
...
fw.ajax.callRemoteFile = function (url,callBack){
	var u = fw.path + "server/getFile." + fw.arg.env;
	return fw.ajax.callFile(u+"?url="+url,callBack);
}
...
fw.ajax.result = function(xmlHTTP){
	if (xmlHTTP.status==0||xmlHTTP.status==200){
		var txt = fw.isie ? fw.lang.bin2ascii(xmlHTTP.responseBody) : xmlHTTP.responseText;
...
fw.array.each = function(a, cb, r){
	if(r) for(var i=0,t,l=a.length;i<l;i++)(t=cb(a[i]))!=undefined&&r.push(t);
	else for(var i=a.length-1;i>=0;i--)(a[i]=cb(a[i]))==undefined&&a.splice(i,1);
	return r||a;
}
...
//数字排序
fw.array.sortNumber = function (a,b){
	return a - b;
}

//数字降序
fw.array.sortNumberDesc = function (a,b){
	return b - a;
}
...
fw.dom.createCSS = function(css){
	return fw.append(fw.head,"link",{type:"text/css",rel:"stylesheet",href:css});
}

//创建JS
fw.dom.createJS = function(js){
	return fw.append(fw.head,"script",{type:"text/javascript",src:js});
}
...
//insertRows
fw.dom.insertRows2 = function (tb,html){
    var o = document.createElement("div");
    o.innerHTML = "<table><tbody>"+html+"</tbody></table>";
    var ol = o.childNodes[0].tBodies[0].rows;
    while(ol.length>0){
...
fw.object.each = function(obj, callBack){
	for (var i in obj){
		callBack(obj[i]);
	}
}
...
fw.re.mobile = function (condition) {
    return fw.re.parse(/1(?:3\d|5\d)-?\d{8}/, condition);
}
...
//tab切换
fw.ui.tb = function (ol, css1, css2, cb){
	ol = fw.childNodes(ol);
	var idx = 0;
	var f = function(i){
		return function (){
...
fw.xml.callXml = function (url,callBack,isDom){
	...
	xmlDom.async = true;			
	if (fw.isie){
		xmlDom.onreadystatechange = function(){	xmlDom.readyState==4 && callBack(f(xmlDom))	};
	}else{
		xmlDom.onload = function(){ callBack(f(xmlDom)) };
	}
	xmlDom.load(url);
	return xmlDom;
}
...
fw.xml.getAtt = function (){
	var json = {};
	for (var i=1,a=arguments,node=a[0],l=a.length;i<l;i++){
		json[a[i]] = node.getAttribute(a[i]) || false;
	}
	return json;
}
...
fw.load = function (module){
	fw.loadJs(fw.path + "com/" + module +  ".js");
}
//加载CSS文件
